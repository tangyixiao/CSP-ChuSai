...
\ans{17.}{A（正确）}\expn{程序第 19 行把 \texttt{len} 初始化为 \texttt{n}，第 20 行第一次累加整个字符串长度 \texttt{n}，之后才沿 \texttt{pi} 回退。改为从 \texttt{pi[n-1]} 开始后，只少累加这一项，因此输出恰好减少 \texttt{n}。}
\ans{18.}{B（错误）}\expn{程序第 12 行在失配后可能连续沿着 \texttt{pi} 回退，第 14--16 行再完成匹配和赋值。把第 12 行的 \texttt{while} 改成 \texttt{if} 后只能回退一次，某些字符串的前缀函数会被算错，第 21 行的最终输出也可能改变。}
...
\ans{21.}{A（正确）}\expn{程序第 16--18 行枚举所有 $m$ 位状态，只保留没有相邻 1 的状态。三列中符合条件的状态为 \texttt{000,001,010,100,101}，共 5 个。}
\ans{22.}{A（正确）}\expn{程序第 29--32 行枚举上一行状态并判断两行是否冲突。本题 \texttt{s=101}、\texttt{t=001} 时，\texttt{101\&001=001}，第 30 行的判断结果非零，于是执行第 31 行的 \texttt{continue}，不会执行第 32 行的转移累加。}
\ans{23.}{B（错误）}\expn{程序第 25 行清空当前行对应的滚动数组，第 26--34 行再向其中写入本轮转移结果。删去第 25 行后，上一轮保留的数会被继续累加，某些输入的方案数会变大。}
...
\ans{26.}{B}\expn{程序第 23、24 行把两个滚动数组下标交换回来，使 \texttt{pre} 指向上一轮、\texttt{cur} 指向本轮；第 25 行清空本轮数组。删去第 23、24 行后，第一轮会清空原有的 \texttt{f[0][0]=1}，之后从全为 0 的数组转移，最终由第 44 行输出 0。}

\subsection*{（3）区间仿射更新线段树}
\overview{每个结点保存所代表区间的总和。一次完整覆盖的更新可以先记在当前结点，之后访问子区间时再传给两个子结点。}
\ans{27.}{A（正确）}\expn{程序第 37--40 行是 \texttt{upd} 的完整覆盖分支：当前结点区间被完全包含时，第 39 行调用一次 \texttt{apply} 并执行第 40 行的 \texttt{return}，不会继续访问子结点。}
\ans{28.}{B（错误）}\expn{程序第 32、33 行把当前结点的懒标记下传给左右子结点，第 34、35 行分别把 \texttt{mul} 和 \texttt{add} 重置。删去第 35 行后，旧的加法标记会在下次执行第 32、33 行时再次下传，可能被重复计算，导致查询结果错误。}
\ans{29.}{A（正确）}\expn{程序第 12--22 行建树为 $O(n)$，第 37--49 行的每次更新和第 50--61 行的每次查询均为 $O(\log n)$，总量可写成 $O(n+q\log n)$，也属于 $O((n+q)\log n)$。}
\ans{30.}{B}\expn{程序第 24 行更新区间总和。区间中共有 \texttt{r-l+1} 个数，每个数先乘 \texttt{k} 再加 \texttt{b}，所以总和变为 \texttt{tr[p].sum*k+(r-l+1)*b}，最后按模数取模。}
\ans{31.}{A}\expn{程序第 32、33 行分别对左、右子结点调用 \texttt{apply}。交换这两个调用的先后顺序只改变访问顺序，不改变两个子结点得到的标记和结果；其余三项都会丢失或重复应用部分更新。}
