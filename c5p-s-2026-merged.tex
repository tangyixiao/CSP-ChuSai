...
\q{26.} 若删去主循环中的两条语句 \texttt{pre \^= mask;} 和 \texttt{cur \^= mask;}，程序的输出将（\qquad）\\
\opt{A. 只有第一行输入的 \texttt{n,m} 会影响输出，棋盘各行的内容不会影响输出}
\opt{B. 输出为 0}
...
\opt{A. 正确}\opt{B. 错误}
\q{28.} 若把 \texttt{push} 末尾的 \texttt{tr[p].add} 重置语句删去，则程序对所有输入的输出仍然不变。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
