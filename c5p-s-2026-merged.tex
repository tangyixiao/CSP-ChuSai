...
\q{16.} 当输入字符串为 \texttt{ababa} 时，程序计算得到 \texttt{*(mark+4)=3}。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\begin{minipage}{\linewidth}
\q{17.} 输入字符串为 \texttt{ababa}，程序执行第 34--35 行的累加循环后，输出为（\qquad）\\
\opt{A. 7}\opt{B. 8}\opt{C. 9}\opt{D. 10}
\end{minipage}
\q{18.} 输入字符串为 \texttt{bbba}。第 17 行的 \texttt{while} 保持不变时程序输出为 \texttt{X}；只将第 17 行的 \texttt{while} 改为 \texttt{if} 时程序输出为 \texttt{Y}。二元组 \texttt{(X,Y)} 为（\qquad）\\
...
\noindent\wdot\hspace{0.5em}判断题\\
\q{21.} 当 \texttt{m=3} 时，经过第 16--18 行的枚举后，\texttt{st.size()} 的值为 5。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\begin{minipage}{\linewidth}
...
void build(int p, int l, int r) {
    ...
    build(p << 1, l, m);
    build(p << 1 | 1, m + 1, r);
    info[p].sum = (info[p << 1].sum + info[p << 1 | 1].sum) % P;
}
void act(int p, int l, int r, ll k, ll b) {
    info[p].sum = (info[p].sum * k + (r - l + 1) * b) % P;
    info[p].x = info[p].x * k % P;
    info[p].y = (info[p].y * k + b) % P;
}
void visit(int p, int l, int r) {
    if (l == r)
        return;
    int m = (l + r) >> 1;
    act(p << 1, l, m, info[p].x, info[p].y);
    act(p << 1 | 1, m + 1, r, info[p].x, info[p].y);
    std::exchange(info[p].x, 1);
    std::exchange(info[p].y, 0);
}
void change(int p, int l, int r, int ql, int qr, ll k, ll b) {
    if (ql <= l && r <= qr) {
        act(p, l, r, k, b);
        return;
    }
    ...
    int m = (l + r) >> 1;
    if (ql <= m)
        change(p << 1, l, m, ql, qr, k, b);
    if (qr > m)
        change(p << 1 | 1, m + 1, r, ql, qr, k, b);
    info[p].sum = (info[p << 1].sum + info[p << 1 | 1].sum) % P;
}
ll ask(int p, int l, int r, int ql, int qr) {
    ...
    ll s = 0;
    if (ql <= m)
        s = (s + ask(p << 1, l, m, ql, qr)) % P;
    if (qr > m)
        s = (s + ask(p << 1 | 1, m + 1, r, ql, qr)) % P;
    return s;
}
int main() {
    cin >> n >> q;
...
\noindent\wdot\hspace{0.5em}判断题\\
\begin{minipage}{\linewidth}
\q{27.} 在执行一次区间修改时，如果当前结点区间完全包含在修改范围内，程序会执行第 38--40 行：先处理当前结点，再直接返回，不再访问该结点的子结点。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\end{minipage}
\q{28.} 若把第 34--35 行的第二条 \texttt{std::exchange} 语句删去，程序对所有输入的输出仍然不变。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{29.} 程序的时间复杂度为 \(O((n+q)\log n)\)。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}

\noindent\wdot\hspace{0.5em}单选题\\
\q{30.} 设当前结点 \texttt{p} 表示区间 \([l,r]\)，执行第 39 行的 \texttt{act(p,l,r,k,b)} 后，\texttt{info[p].sum} 的新值为（\qquad）\\
\opt{A. \texttt{(info[p].sum*k+b) \% P}}
\opt{B. \texttt{(info[p].sum*k+(r-l+1)*b) \% P}}
\opt{C. \texttt{((info[p].sum+b)*k) \% P}}
\opt{D. \texttt{((r-l+1)*(info[p].sum*k+b)) \% P}}
\begin{minipage}{\linewidth}
\q{31.} 关于第 34--35 行的两条 \texttt{std::exchange} 语句，下列修改中不会改变程序输出的是（\qquad）\\
\opt{A. 交换第 32、33 行两次 \texttt{act} 调用的先后顺序}
\opt{B. 在两次 \texttt{act} 调用前，先执行第 35 行的语句}
\opt{C. 把两次 \texttt{act} 调用中的参数 \texttt{b} 都改为 0}
\opt{D. 删除第 33 行的 \texttt{act} 调用，但保留第 32 行的调用}
\end{minipage}
...
\q{33.} 第18行①处应填（\qquad）\\
\optw{A. \texttt{seen[t][i-1]}}{B. \texttt{seen[t][i] + 1}}
\optw{C. \texttt{seen[t][i-1] + (t == c)}}{D. \texttt{seen[c][i-1]}}
\q{34.} 第23行②处应填（\qquad）\\
