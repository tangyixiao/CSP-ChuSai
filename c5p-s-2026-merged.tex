...
\opt{A. 正确}\opt{B. 错误}
\begin{minipage}{\linewidth}
\q{22.} 在第 30--31 行的某次转移中，若当前状态 \texttt{s=101}、上一行状态 \texttt{t=001}，则程序会执行 \texttt{continue}，不会把 \texttt{f[pre][t]} 加入当前状态。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\end{minipage}
...
\noindent\wdot\hspace{0.5em}单选题\\
\q{30.} 设当前结点 \texttt{p} 表示区间 \([l,r]\)，执行第 24 行的 \texttt{apply(p,l,r,k,b)} 后，\texttt{tr[p].sum} 的新值为（\qquad）\\
\opt{A. \texttt{(tr[p].sum*k+b) \% P}}
\opt{B. \texttt{(tr[p].sum*k+(r-l+1)*b) \% P}}
\opt{C. \texttt{((tr[p].sum+b)*k) \% P}}
\opt{D. \texttt{((r-l+1)*(tr[p].sum*k+b)) \% P}}
\q{31.} 关于第 32--35 行的函数 \texttt{push}，下列修改中不会改变程序输出的是（\qquad）\\
