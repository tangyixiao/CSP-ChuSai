...
\opt{A. 正确}\opt{B. 错误}
\begin{minipage}{\linewidth}
\q{17.} 输入字符串为 \texttt{ababa}，程序执行第 22--23 行的累加循环后，输出为（\qquad）\\
\opt{A. 7}\opt{B. 8}\opt{C. 9}\opt{D. 10}
\end{minipage}
\q{18.} 输入字符串为 \texttt{bbba}。第 15 行的 \texttt{while} 保持不变时程序输出为 \texttt{X}；只将第 15 行的 \texttt{while} 改为 \texttt{if} 时程序输出为 \texttt{Y}。二元组 \texttt{(X,Y)} 为（\qquad）\\
