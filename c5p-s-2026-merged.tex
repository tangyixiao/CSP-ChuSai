...
\noindent\wdot\hspace{0.5em}判断题\\
\begin{minipage}{\linewidth}
\q{27.} 若第 38 行的条件 \texttt{ql <= l \&\& r <= qr} 成立，程序会执行第 39 行，然后在第 40 行直接返回，不再继续处理左右两部分。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\end{minipage}
\q{28.} 若把第 35 行的语句删去，程序对所有输入的输出仍然不变。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{29.} 程序的时间复杂度为 \(O((n+q)\log n)\)。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}

\noindent\wdot\hspace{0.5em}单选题\\
\q{30.} 执行第 39 行时，若 \texttt{p} 表示区间 \([l,r]\)，执行后 \texttt{info[p].sum} 的新值为（\qquad）\\
...
\begin{minipage}{\linewidth}
\q{31.} 关于第 34--35 行的两条语句，下列修改中不会改变程序输出的是（\qquad）\\
\opt{A. 交换第 32、33 行两次调用的先后顺序}
\opt{B. 在两次调用前，先执行第 35 行的语句}
\opt{C. 把两次调用中的参数 \texttt{b} 都改为 0}
\opt{D. 删除第 33 行的调用，但保留第 32 行的调用}
\end{minipage}
