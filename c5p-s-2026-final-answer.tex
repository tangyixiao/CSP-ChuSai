...
\ans{17.}{A（正确）}\expn{先统计每个位置的 \texttt{pi}，再按长度从大到小向父前缀累加，最后加上前缀自身出现的一次。}
\ans{18.}{A（正确）}\expn{块注释从 \texttt{/\char42} 开始，到下一行的 \texttt{\char42/} 结束；\texttt{len *= 4} 被注释掉，实际返回 $3-(1+2)=0$。}
...
\ans{22.}{A（正确）}\expn{程序只判断上下相邻行的位掩码是否相交，对角线相邻不构成冲突。}
\ans{23.}{A（正确）}\expn{无相邻 1 的 10 位掩码数量为 $F_{12}=144$。}
\ans{24.}{B}\expn{预处理状态为 $O(2^m)$，每行枚举状态对，复杂度为 $O(nV^2+2^m)$。}
\ans{25.}{C}\expn{两行三列全可选时，按行状态转移统计得到 17。}
\ans{26.}{C}\expn{四行四列全可选时，统计得到 1234。}

\subsection*{（3）区间仿射更新线段树}
\overview{结点维护区间和、乘法懒标记和加法懒标记；更新 $x\mapsto kx+b$ 时，区间和与两个标记同步合成。}
\ans{27.}{A（正确）}\expn{先做 $k_1x+b_1$ 再做 $k_2x+b_2$，结果为 $k_1k_2x+b_1k_2+b_2$。}
\ans{28.}{B（错误）}\expn{仿射变换一般不交换，例如先乘后加与先加后乘的结果不同。}
\ans{29.}{A（正确）}\expn{建树为 $O(n)$，每次更新或查询为 $O(\log n)$，总量可写成 $O(n+q\log n)$，也属于 $O((n+q)\log n)$。}
\ans{30.}{C}\expn{更新后数组为 \texttt{1 5 7 9}，总和为 22。}
\ans{31.}{C}\expn{两次更新后数组为 \texttt{3 5 11 13 8 8}，区间 $[2,5]$ 的和为 35。}
\ans{32.}{C（4 分）}\expn{依次更新后区间 $[2,6]$ 的和为 103。}

\section*{三、完善程序（33--42）}
\subsection*{（1）三色序列}
\ans{33.}{B}\expn{新字符须与上一个字符不同，且该颜色仍有剩余，故选 B。}
\ans{34.}{B}\expn{要取该颜色尚未使用的下一个位置，应为 \texttt{pos[c][used[c]+1]}。}
\ans{35.}{A}\expn{把原位置为 \texttt{x} 的字符移到当前构造串末尾，基础位移量为 \texttt{x-len-1}。}
\ans{36.}{B}\expn{已经使用但位于 \texttt{x} 之后的字符会增加交换量，增加量为 \texttt{max(0,used[t]-pre[t][x])}。}
