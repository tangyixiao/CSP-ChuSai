...
% 编译方式：xelatex c5p-s-2026-answer.tex（建议编译两次）
...
\end{tikzpicture}
\end{center}
\ans{16.}{A}\expn{递归范围是闭区间 $[1,4]$，因此前四个元素被排为 $1,2,3,4$；下标 5 不在本次调用范围内，原本的 5 保持不变。}
\ans{17.}{A（√）}\expn{判断条件是 \texttt{a[i] <= a[j]}，相等时执行 \texttt{tmp[k++] = a[i++]}，所以左区间元素先写入。}
\ans{18.}{B（×）}\expn{当 $l=r$ 时函数直接返回，并没有给 \texttt{tmp[l]} 赋值；所以不能对“任意一次”返回的调用都断言 \texttt{tmp[l..r]} 有序。}
\ans{19.}{A（√）}\expn{每层把区间长度约减半，递归树高度为 $O(\log n)$。}
\ans{20.}{B}\expn{$(3+10)>>1=6$，递归调用的两个闭区间分别为 $[3,6]$ 与 $[7,10]$。}
\ans{21.}{C}\expn{最坏情况下 $T(1)=0$，且 $T(n)=2T(n/2)+(n-1)$。因此 $T(8)=2T(4)+7=2(2T(2)+3)+7=17$。}
...
\end{tabular}
\end{center}
\ans{22.}{A（√）}\expn{当 \texttt{k=0} 时，扫描过程中的弹栈条件第一项始终为假；扫描结束后的删除循环也不会执行。}
\ans{23.}{B（×）}\expn{应在 \texttt{st.back() > ch} 时弹栈。若栈顶更小，保留更小字符在前面反而更优。}
\ans{24.}{A（√）}\expn{若扫描结束仍有删除次数，说明栈中已经没有可改善的前大后小位置；继续删除末尾不会改变已确定的更优前缀。}
\ans{25.}{B}\expn{处理 \texttt{d,c,a} 后还剩一次删除机会；随后栈依次为 \texttt{ab}、\texttt{abc}、\texttt{abcc}，遇到下一个 \texttt{b} 时弹出一个 \texttt{c} 再压入 \texttt{b}，最后压入 \texttt{a}，最大长度为 5。}
\ans{26.}{A}\expn{依次处理后栈状态为 \texttt{c}、\texttt{a}、\texttt{ab}、\texttt{abc}、\texttt{abcc}、\texttt{abcb}、\texttt{abcba}；三次删除机会已用完，最终输出 \texttt{abcba}。}
...
\end{center}
\ans{28.}{D}\expn{邻接表采用头插法，结点 1 的访问顺序为 3、2；随后按队列顺序访问 6、5、4、7，距离依次达到 1、2、3，最终 \texttt{far} 为 7。}
\ans{29.}{A（√）}\expn{更新条件是 \texttt{dista[u] > dista[far]}，而不是大于等于；距离相同时不会覆盖原来的 \texttt{far}，因此保留先出队的结点。}
\ans{30.}{A（√）}\expn{区间 \texttt{dista+1} 到 \texttt{dista+n} 被初始化，\texttt{dista[0]} 虽未处理，但程序只访问编号 1 到 \texttt{n} 的结点。}
\ans{31.}{B}\expn{当前队列尾位置为 \texttt{r}，先把 \texttt{v} 写到 \texttt{que[r]}，再令 \texttt{r} 加一，合写为 \texttt{que[r++] = v;}。}
\ans{32.}{A（√）}\expn{最多有 $n-1$ 条无向边，每条边调用两次 \texttt{add}，所以最多产生 $2(n-1)$ 个邻接项；而 $N=2\times10^5+5$，数组容量足够。}
\ans{33.}{C（4 分）}\expn{该树的一条最长路径为 $7\to6\to3\to1\to2\to4$，包含 5 条边，因此第二次 BFS 输出 5。}
