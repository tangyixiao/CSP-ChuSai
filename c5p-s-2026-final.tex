...
\opt{A. 正确}\opt{B. 错误}
\q{17.} 执行求解 \texttt{pi} 的循环 \texttt{for (int i = 1; i < n; ++i)} 结束后，\texttt{pi[0]} 到 \texttt{pi[n-1]} 已经全部求出。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{18.} 输入字符串为 \texttt{ababa}。执行统计 \texttt{occ} 的三段循环后，\texttt{occ[3]} 等于 2。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
...
\opt{A. 正确}\opt{B. 错误}
\q{22.} 处理第 \(i\) 行时，令 \texttt{s} 表示第 \(i\) 行的选取状态、\texttt{t} 表示第 \(i-1\) 行的选取状态。若第 \(i\) 行第 \(j\) 列与第 \(i-1\) 行第 \(j+1\) 列同时被选中，程序不会因这两个对角线相邻位置而跳过该状态转移。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{23.}（2 分）处理第 \(i\) 行时，执行 \texttt{pre=i\&1}、\texttt{cur=pre XOR 1} 及后面的两次下标交换后，\texttt{pre} 指向第 \(i-1\) 行结果所在的滚动数组，\texttt{cur} 指向第 \(i\) 行要写入的滚动数组。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
...
\opt{A. 正确}\opt{B. 错误}
\q{28.} 假设主函数代码第 55 行依次执行两条更新：对区间 \([2,4]\) 先执行 \(x\leftarrow 2x+1\)，再执行 \(x\leftarrow x+3\)。若只交换这两次 \texttt{upd(...)} 调用的先后顺序，最终数组一定不变。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{29.} 程序的时间复杂度为 \(O((n+q)\log n)\)。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}

\noindent\wdot\hspace{0.5em}单选题\\
\q{30.} 当 \texttt{upd} 函数执行到代码第 32 行，且当前结点区间 \([l,r]\) 未被更新区间 \([ql,qr]\) 完全包含时，\texttt{push(p,l,r)} 的作用是（\qquad）\\
\opt{A. 将当前结点的懒标记表示的更新下传给两个子结点，再递归处理可能相交的子区间}
\opt{B. 将两个子结点的区间和直接赋给当前结点，并清空子结点}
\opt{C. 将更新区间扩大为当前结点区间，使当前结点可以直接调用 \texttt{apply}}
\opt{D. 只在查询操作中统计当前结点的区间和，不改变任何懒标记}
\q{31.} 当 \texttt{qry} 函数执行到代码第 39 行且 \texttt{ql <= l \&\& r <= qr} 成立时，程序直接返回 \texttt{tr[p].sum}，不执行第 40 行的 \texttt{push(p,l,r)}。这样做的原因是（\qquad）\\
\opt{A. 当前结点的区间和已经包含其待下传更新，且查询不需要访问该结点的子结点}
\opt{B. 查询操作不需要考虑任何懒标记，子结点中的值一定比当前结点准确}
\opt{C. 只有叶结点才允许返回 \texttt{tr[p].sum}，所以此处必然不会继续递归}
\opt{D. \texttt{push} 会改变查询区间，因此完全包含时必须跳过所有区间更新}
