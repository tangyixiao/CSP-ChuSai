...
\q{3.} 某递归算法处理规模为 \texttt{n} 的问题时，会递归处理两个规模均为 \texttt{n/2} 的子问题；除两次递归调用外，本次调用还需要 \(\Theta(n\log n)\) 次基本操作。设 \texttt{n} 为 2 的幂且 \(T(1)=\Theta(1)\)，则由主定理可知该算法的时间复杂度为（\qquad）\\
...
\q{12.} 在整数 \texttt{1,2,...,1000} 中，恰好能被 \texttt{2,3,5} 中的一个数整除的整数共有（\qquad）个\\
...
\]
若 \texttt{0 <= x < 504}，则 \texttt{x} 的值为（\qquad）\\
\opt{A. 247}\opt{B. 355}\opt{C. 499}\opt{D. 503}

\q{15.} 对任意一个连通无向带权图，下列说法中一定正确的是（\qquad）\\
...
\q{16.} 若调用前 \texttt{ans=0}，调用 \texttt{merge\_sort(1,n)} 后，\texttt{ans} 等于原数组的逆序对数量。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{17.} 对于下标满足 \texttt{i<j} 且 \texttt{a[i]=a[j]} 的两个元素，这一对元素会对 \texttt{ans} 贡献 1。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{18.} 若把 \texttt{ans} 的类型改成 32 位有符号 \texttt{int}，则在题目给定的数组上界 \texttt{n=2e5} 下可能发生溢出。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}

\noindent\wdot\hspace{0.5em}单选题\\
\q{19.} 若初始数组为 \texttt{[4,1,3,2,5]}，只调用 \texttt{merge\_sort(1,4)}，调用返回后 \texttt{a[1..5]} 为（\qquad）\\
...
\opt{A. 正确}\opt{B. 错误}
\q{23.} 扫描完整个字符串后，如果仍有 \texttt{k>0}，继续从当前序列末尾删除字符可以得到字典序最小的结果。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{24.} 虽然程序中存在嵌套的 \texttt{while}，但每个字符最多入栈一次、出栈一次，因此程序的总时间复杂度为 \texttt{O(n)}。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}

\noindent\wdot\hspace{0.5em}单选题\\
\q{25.} 输入为 \texttt{9 4 cbacdcbca}。扫描完前 6 个字符后，\texttt{st} 从底到顶的内容以及此时 \texttt{k} 的值分别为（\qquad）\\
\opt{A. \texttt{acd, 2}}\opt{B. \texttt{acc, 1}}\opt{C. \texttt{acb, 0}}\opt{D. \texttt{abc, 1}}
\q{26.} 当输入为 \texttt{8 3 dcabccba} 时，扫描字符串的过程中 \texttt{st} 的最大长度为（\qquad）\\
\opt{A. 3}\opt{B. 4}\opt{C. 5}\opt{D. 6}
\q{27.} 当输入为 \texttt{8 3 dcabccba} 时，程序的最终输出为（\qquad）\\
...
\noindent\wdot\hspace{0.5em}判断题\\
\q{28.} 若 BFS 中有多个结点与起点距离同为最大值，则函数会保留这些结点中最先从队列里取出的那个作为 \texttt{far}。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{29.} 对同一棵树，仅改变输入边的先后顺序，第一次调用 \texttt{bfs(1)} 的返回值可能发生改变。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{30.} 对任意一棵树，从任意结点进行一次 BFS 得到某个最远点 \texttt{s}，再从 \texttt{s} BFS 得到最远点 \texttt{t}，则 \texttt{dista[t]} 等于该树的直径长度。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}
\q{31.} 若把输入从“树”放宽为任意连通无权图，则仍可保证上述“两次 BFS”方法得到图的直径。（\qquad）\\
\opt{A. 正确}\opt{B. 错误}

\noindent\wdot\hspace{0.5em}单选题\\
\q{32.} 若输入边依次为 \texttt{1-2,1-3,2-4,2-5,3-6,3-7}，则调用 \texttt{bfs(1)} 返回的结点为（\qquad）\\
\opt{A. 4}\opt{B. 5}\opt{C. 6}\opt{D. 7}
...
\noindent（2）（最近公共祖先）给定一棵以 1 为根、包含 \texttt{n} 个结点的树，并回答 \texttt{q} 次询问，每次给出两个结点 \texttt{u,v}，求它们的最近公共祖先。输入满足 \texttt{1 <= n,q <= 2 * 10\string^5}。下面程序使用倍增法预处理祖先信息。
\begin{codebox}\begin{lstlisting}
#include <bits/stdc++.h>
using namespace std;
